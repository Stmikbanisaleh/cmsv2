...
class Administrator extends CI_Controller {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array('id_kategori' => $this->db->escape_str($this->input->post('a')),
                    ...
                    'judul_e' => $this->db->escape_str($this->input->post('bb')),
                    ...
                    'isi_berita_e' => $this->input->post('hh'),
                    'keterangan_gambar' => $this->input->post('i'),
                    'keterangan_gambar_e' => $this->input->post('ii'),
                    'hari' => hari_ini(date('w')),
                    'tanggal' => date('Y-m-d'),
                    'jam' => date('H:i:s'),
                    'dibaca' => '0',
    ...
    // Controller Modul List Artikel

    function listartikel() {
        cek_session_akses('listartikel', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('artikel', 'id_artikel', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('artikel', array('username' => $this->session->username), 'id_artikel', 'DESC');
        }
        $data['rss'] = $this->model_utama->view_joinn('artikel', 'users', 'kategori', 'username', 'id_kategori', 'id_artikel', 'DESC', 0, 10);
        $data['iden'] = $this->model_utama->view_where('identitas', array('id_identitas' => 1))->row_array();
        $this->load->view('administrator/rss', $data);
        $this->template->load('administrator/template', 'administrator/mod_artikel/view_artikel', $data);
    }

    function tambah_listartikel() {
        cek_session_akses('listartikel', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_artikel/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('k');
            ...
            $config['source_image'] = 'asset/foto_artikel/' . $hasil['file_name'];
            $config['wm_text'] = 'phpmu.com';
            $config['wm_type'] = 'text';
            $config['wm_font_path'] = './system/fonts/texb.ttf';
            $config['wm_font_size'] = '26';
            ...
            if ($hasil['file_name'] == '') {
                $data = array('id_kategori' => $this->db->escape_str($this->input->post('a')),
                    ...
                    'aktif' => $this->db->escape_str($this->input->post('f')),
                    'utama' => $this->db->escape_str($this->input->post('g')),
                    'isi_artikel' => $this->input->post('h'),
                    'isi_artikel_e' => $this->input->post('hh'),
                    'keterangan_gambar' => $this->input->post('i'),
                    'keterangan_gambar_e' => $this->input->post('ii'),
                    'hari' => hari_ini(date('w')),
                    'tanggal' => date('Y-m-d'),
            ...
            $this->model_app->insert('artikel', $data);
            redirect($this->uri->segment(1) . '/listartikel');
        } else {
            $data['tag'] = $this->model_app->view_ordering('tag', 'id_tag', 'DESC');
            $data['record'] = $this->model_app->view_ordering('kategori', 'id_kategori', 'DESC');
    ...
    function edit_listartikel() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_artikel' => $this->input->post('id'));
            $this->model_app->update('artikel', $data, $where);
            redirect($this->uri->segment(1) . '/listartikel');
        } else {
            $tag = $this->model_app->view_ordering('tag', 'id_tag', 'DESC');
            $record = $this->model_app->view_ordering('kategori', 'id_kategori', 'DESC');
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('artikel', array('id_artikel' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('artikel', array('id_artikel' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses, 'tag' => $tag, 'record' => $record);
    ...
    function publish_listartikel() {
        ...
        $where = array('id_artikel' => $this->uri->segment(3));
        $this->model_app->update('artikel', $data, $where);
        redirect($this->uri->segment(1) . '/listartikel');
    }

    function delete_listartikel() {
        cek_session_akses('listartikel', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_artikel' => $this->uri->segment(3));
        } else {
            $id = array('id_artikel' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('artikel', $id);
        redirect($this->uri->segment(1) . '/listartikel');
    }
    ...
    function project_finished() {
        cek_session_akses('project', $this->session->id_session);
        $data['record'] = $this->model_app->view_where_ordering('project', array('status' => 1), 'id_project', 'DESC');
        $this->template->load('administrator/template', 'administrator/mod_project/view_project', $data);
    }
    
    function project_ongoing() {
        cek_session_akses('project', $this->session->id_session);
        $data['record'] = $this->model_app->view_where_ordering('project', array('status' => 1), 'id_project', 'DESC');
        $this->template->load('administrator/template', 'administrator/mod_project/view_project', $data);
    }
    ...
    function tambah_project() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $hasil = $this->upload->data();
            if ($hasil['file_name'] == '') {
                $data = array('nm_project' => $this->input->post('a'),
                    'user_input' => $this->session->username,
                    'nm_project_seo' => seo_title($this->input->post('a')),
                    'tgl_mulai' => $this->input->post('b'),
                    'tgl_selesai' => $this->input->post('c'),
                    'description' => $this->input->post('d'),
                    'description_eng' => $this->input->post('e'),
                    'status' => $this->input->post('f'),
                    'url_project' => $this->input->post('h')
                        );
            } else {
                $data = array('nm_project' => $this->input->post('a'),
                    'nm_project_seo' => seo_title($this->input->post('a')),
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array('nm_project' => $this->input->post('a'),
                    ...
                    'tgl_selesai' => $this->input->post('c'),
                    'description' => $this->input->post('d'),
                    'description_eng' => $this->input->post('ee'),
                    'status' => $this->input->post('e'),
                    'url_project' => $this->input->post('h'));
            } else {
                $data = array('nm_project' => $this->input->post('a'),
    ...
    //social media
    function sosmed() {
        cek_session_akses('sosmed', $this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('sosmed', 'id_sosmed', 'DESC');
        $this->template->load('administrator/template', 'administrator/mod_sosmed/view_sosmed', $data);
    }

    function tambah_sosmed() {
        cek_session_akses('sosmed', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_sosmed/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            ...
            $hasil = $this->upload->data();
            if ($hasil['file_name'] == '') {
                $data = array('link' => $this->input->post('a'),
                    'class_boostrap' => $this->input->post('c'),
                    'status' => $this->input->post('d'),
                    'title' => $this->input->post('aa'));
            } else {
                $data = array(
            ...
            $this->model_app->insert('sosmed', $data);
            redirect($this->uri->segment(1) . '/sosmed');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_sosmed/view_sosmed_tambah');
        }
    }
    ...
    function edit_sosmed() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_sosmed' => $this->input->post('id'));
            $this->model_app->update('sosmed', $data, $where);
            redirect($this->uri->segment(1) . '/sosmed');
        } else {
            $proses = $this->model_app->edit('sosmed', array('id_sosmed' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_sosmed/view_sosmed_edit', $data);
        }
    }

    function delete_sosmed() {
        cek_session_akses('sosmed', $this->session->id_session);
        $id = array('id_sosmed' => $this->uri->segment(3));
        $this->model_app->delete('sosmed', $id);
        redirect($this->uri->segment(1) . '/sosmed');
    }

    function shipdesign() {
        cek_session_akses('shipdesign', $this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('shipdesign', 'id_shipdesign', 'DESC');
        $this->template->load('administrator/template', 'administrator/mod_shipdesign/view_shipdesign', $data);
    }

    function tambah_shipdesign() {
        cek_session_akses('shipdesign', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_project/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|PDF|RAR';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('b');
            $hasil = $this->upload->data();
            if ($hasil['file_name'] == '') {
                $data = array(
                    'user_input' => $this->session->username,
                    'description' => $this->input->post('a'),
                    'description_eng' => $this->input->post('c'));
            } else {
                $data = array(
                    'user_input' => $this->session->username,
                    'description' => $this->input->post('a'),
            ...
            $this->model_app->insert('shipdesign', $data);
            redirect($this->uri->segment(1) . '/shipdesign');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_shipdesign/view_shipdesign_tambah');
        }
    }

    function edit_shipdesign() {
        cek_session_akses('project', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_project/';
            ...
            $where = array('id_shipdesign' => $this->input->post('id'));
            $this->model_app->update('shipdesign', $data, $where);
            redirect($this->uri->segment(1) . '/edit_shipdesign/1');
        } else {
            $proses = $this->model_app->edit('shipdesign', array('id_shipdesign' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_shipdesign/view_shipdesign_edit', $data);
        }
    }

    function delete_shipdesign() {
        cek_session_akses('shipdesign', $this->session->id_session);
        $id = array('id_shipdesign' => $this->uri->segment(3));
        $this->model_app->delete('shipdesign', $id);
        redirect($this->uri->segment(1) . '/shipdesign');
    }

    //isps code
    function edit_ispscode() {
        cek_session_akses('ispscode', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_project/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            ...
            $where = array('id_ispscode' => $this->input->post('id'));
            $this->model_app->update('ispscode', $data, $where);
            redirect($this->uri->segment(1) . '/edit_ispscode/1');
        } else {
            $proses = $this->model_app->edit('ispscode', array('id_ispscode' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_ispscode/view_ispscode_edit', $data);
        }
    }

    //konsultasi code
    function edit_konsultasi() {
        cek_session_akses('konsultasi', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_project/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'user_input' => $this->session->username,
                    'description' => $this->input->post('a'),
                    'description_eng' => $this->input->post('b'));
            } else {
                $data = array(
                    'user_input' => $this->session->username,
                    'description' => $this->input->post('a'),
            ...
            $where = array('id_konsultasi' => $this->input->post('id'));
            $this->model_app->update('konsultasi', $data, $where);
            redirect($this->uri->segment(1) . '/edit_konsultasi/1');
        } else {
            $proses = $this->model_app->edit('konsultasi', array('id_konsultasi' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_konsultasi/view_konsultasi_edit', $data);
        }
    }

    //supervisi
    function edit_supervisi() {
        cek_session_akses('supervisi', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_project/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            ...
            $where = array('id_supervisi' => $this->input->post('id'));
            $this->model_app->update('supervisi', $data, $where);
            redirect($this->uri->segment(1) . '/edit_supervisi/1');
        } else {
            $proses = $this->model_app->edit('supervisi', array('id_supervisi' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_supervisi/view_supervisi_edit', $data);
        }
    }
    ...
    // Controller Modul Lowongan

    function lowongan() {
        cek_session_akses('lowongan', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('lowongan', 'id_lowongan', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('lowongan', array('username' => $this->session->username), 'id_lowongan', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_lowongan/view_lowongan', $data);
    }

    function tambah_lowongan() {
        cek_session_akses('lowongan', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $data = array('kode_lowongan' => $this->db->escape_str($this->input->post('a')),
                'posisi' => $this->db->escape_str($this->input->post('b')),
                'kriteria' => $this->db->escape_str($this->input->post('c'))
            );
            $this->model_app->insert('lowongan', $data);
            redirect($this->uri->segment(1) . '/lowongan');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_lowongan/view_lowongan_tambah');
        }
    }
    ...
    function edit_lowongan() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_lowongan' => $this->input->post('id'));
            ...
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_lowongan/view_lowongan_edit', $data);
        }
    }
    ...
    function delete_lowongan() {
        ...
        if ($this->session->level == 'admin') {
            ...
            $id = array('id_lowongan' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('lowongan', $id);
        redirect($this->uri->segment(1) . '/lowongan');
    }

    // Controller Modul document

    function document() {
        cek_session_akses('document', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('document', 'id_document', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('document', array('username' => $this->session->username), 'id_document', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_document/view_document', $data);
    }

    function tambah_document() {
        cek_session_akses('document', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $data = array('persyaratan_doc' => $this->db->escape_str($this->input->post('a'))
            );
            $this->model_app->insert('document', $data);
            redirect($this->uri->segment(1) . '/document');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_document/view_document_tambah');
        }
    }
    ...
    function edit_document() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_document' => $this->input->post('id'));
            $this->model_app->update('document', $data, $where);
            redirect($this->uri->segment(1) . '/document');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('document', array('id_document' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('document', array('id_document' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_document/view_document_edit', $data);
        }
    }
    ...
    function delete_document() {
        cek_session_akses('document', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_document' => $this->uri->segment(3));
        } else {
            $id = array('id_document' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('document', $id);
        redirect($this->uri->segment(1) . '/document');
    }

    // Controller Modul Gallery Photo

    function photo() {
        cek_session_akses('gallery_photos', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_join_one('gallery_photos', 'album', 'id_album', 'id_photo', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_join_where('gallery_photos', 'album', 'id_album', array('username' => $this->session->username), 'id_photo', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_gallery_photos/view_galleryphoto', $data);
    }

    function tambah_galleryphoto() {
        cek_session_akses('gallery_photos', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/front/img/img_galeri/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            if ($hasil['file_name'] == '') {
                $data = array('username' => $this->session->username,
                    ...
                    'keterangan' => $this->input->post('b'),
                    'id_album' => $this->db->escape_str($this->input->post('d'))
                );
            } else {
                $data = array('username' => $this->session->username,
                    'created_at' => date('Y-m-d H:i:s'),
            ...
            $this->model_app->insert('gallery_photos', $data);
            redirect($this->uri->segment(1) . '/photo');
        } else {
            $data['record'] = $this->model_app->view_ordering('album', 'id_album', 'DESC');
            $this->template->load('administrator/template', 'administrator/mod_gallery_photos/view_galleryphoto_tambah', $data);
        }
    }

    function edit_galleryphoto() {
        cek_session_akses('gallery_photos', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/front/img/img_galeri/';
            ...
            $where = array('id_photo' => $this->input->post('id'));
            $this->model_app->update('gallery_photos', $data, $where);
            redirect($this->uri->segment(1) . '/photo');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('gallery_photos', array('id_photo' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('gallery_photos', array('id_photo' => $id, 'username' => $this->session->username))->row_array();
            }
            $album = $this->model_app->view_ordering('album', 'id_album', 'DESC');
            $data = array('rows' => $proses, 'record' => $album);
            $this->template->load('administrator/template', 'administrator/mod_gallery_photos/view_galleryphoto_edit', $data);
        }
    }

    function delete_galleryphoto() {
        cek_session_akses('gallery_photos', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_photo' => $this->uri->segment(3));
        } else {
            $id = array('id_photo' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('gallery_photos', $id);
        redirect($this->uri->segment(1) . '/photo');
    }

    // Controller Modul Gallery video

    function galleryvideo() {
        cek_session_akses('gallery_videos', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('gallery_videos', 'id_video', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('gallery_videos', array('username' => $this->session->username), 'id_video', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_gallery_video/view_video', $data);
    }

    function tambah_galleryvideo() {
        cek_session_akses('gallery_videos', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $data = array('username' => $this->session->username,
                'created_at' => date('Y-m-d H:i:s'),
                'username' => $this->session->username,
                'url_video' => $this->input->post('a')
            );
            $this->model_app->insert('gallery_videos', $data);
            redirect($this->uri->segment(1) . '/galleryvideo');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_gallery_video/view_video_tambah');
        }
    }

    function edit_galleryvideo() {
        cek_session_akses('gallery_videos', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $data = array('username' => $this->session->username,
            ...
            $where = array('id_video' => $this->input->post('id'));
            $this->model_app->update('gallery_videos', $data, $where);
            redirect($this->uri->segment(1) . '/galleryvideo');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('gallery_videos', array('id_video' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('gallery_videos', array('id_video' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_gallery_video/view_video_edit', $data);
        }
    }

    function delete_galleryvideo() {
        cek_session_akses('gallery_videos', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_video' => $this->uri->segment(3));
        } else {
        ...
        $this->model_app->delete('gallery_videos', $id);
        redirect($this->uri->segment(1) . '/galleryvideo');
    }

    // Controller Modul Quality Police

    function edit_qualitypolice() {
        cek_session_akses('quality_police', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $data = array(
                'isi' => $this->db->escape_str($this->input->post('a')),
                'isi_e' => $this->db->escape_str($this->input->post('b'))
            );
            $where = array('id_quality_police' => $this->input->post('id'));
            $this->model_app->update('quality_police', $data, $where);
            redirect($this->uri->segment(1) . '/edit_qualitypolice');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('quality_police', array('id_quality_police' => '1'))->row_array();
            } else {
                $proses = $this->model_app->edit('quality_police', array('id_quality_police' => '1', 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_qualitypolice/view_qualitypolice_edit', $data);
        }
    }

    // Controller Modul Achievement

    function achievement() {
        cek_session_akses('achievement', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('achievement', 'id_achievement', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('achievement', array('username' => $this->session->username), 'id_achievement', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_achievement/view_achievement', $data);
    }

    function tambah_achievement() {
        cek_session_akses('achievement', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/front/img/achievement/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            if ($hasil['file_name'] == '') {
                $data = array('judul' => $this->input->post('a'),
                    ...
                    'username' => $this->session->username,
                    'created_at' => date('Y-m-d H:i:s'),
                    'deskripsi_e' => $this->input->post('bb'),
                    'judul_seo' => seo_title($this->input->post('a'))
                );
            } else {
                $data = array('image' => $hasil['file_name'],
                    'judul' => $this->input->post('a'),
                    'deskripsi' => $this->input->post('b'),
                    'username' => $this->session->username,
                    'created_at' => date('Y-m-d H:i:s'),
            ...
            $this->model_app->insert('achievement', $data);
            redirect($this->uri->segment(1) . '/achievement');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_achievement/view_achievement_tambah');
        }
    }
    ...
    function edit_achievement() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_achievement' => $this->input->post('id'));
            $this->model_app->update('achievement', $data, $where);
            redirect($this->uri->segment(1) . '/achievement');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('achievement', array('id_achievement' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('achievement', array('id_achievement' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_achievement/view_achievement_edit', $data);
        }
    }
    ...
    function delete_achievement() {
        cek_session_akses('achievement', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_achievement' => $this->uri->segment(3));
        } else {
            $id = array('id_achievement' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('achievement', $id);
        redirect($this->uri->segment(1) . '/achievement');
    }

    // Controller Modul Vision Mission

    function edit_visimisi() {
        cek_session_akses('vision_mission', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $data = array(
                'vision' => $this->db->escape_str($this->input->post('a')),
                'keterangan_vision' => $this->db->escape_str($this->input->post('b')),
                'mission' => $this->db->escape_str($this->input->post('c')),
                'username' => $this->session->username,
                'keterangan_mission' => $this->db->escape_str($this->input->post('d')),
                'vision_e' => $this->db->escape_str($this->input->post('e')),
                'keterangan_vision_e' => $this->db->escape_str($this->input->post('f')),
                'mission_e' => $this->db->escape_str($this->input->post('g')),
                'keterangan_mission_e' => $this->db->escape_str($this->input->post('h'))
            );
            $where = array('id_vision_mission' => $this->input->post('id'));
            $this->model_app->update('vision_mission', $data, $where);
            redirect($this->uri->segment(1) . '/edit_visimisi');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('vision_mission', array('id_vision_mission' => '1'))->row_array();
            } else {
                $proses = $this->model_app->edit('vision_mission', array('id_vision_mission' => '1', 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_vision_mission/view_vision_mission_edit', $data);
        }
    }

    // Controller Modul Company Moto

    function edit_companymoto() {
        cek_session_akses('company_moto', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $data = array(
                'deskripsi_moto' => $this->input->post('a'),
                'deskripsi_moto_e' => $this->input->post('b')
            );
            $where = array('id_company_moto' => $this->input->post('id'));
            $this->model_app->update('company_moto', $data, $where);
            redirect($this->uri->segment(1) . '/edit_companymoto');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('company_moto', array('id_company_moto' => '1'))->row_array();
            } else {
                $proses = $this->model_app->edit('company_moto', array('id_company_moto' => '1', 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_company_moto/view_company_moto_edit', $data);
        }
    }
    ...
    function edit_history() {
        ...
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $data = array('deskripsi_history' => $this->input->post('a'),
                        'deskripsi_history_e' => $this->input->post('b')
            );
            $where = array('id_history' => $this->input->post('id'));
            $this->model_app->update('history', $data, $where);
            redirect($this->uri->segment(1) . '/edit_history');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('history', array('id_history' => '1'))->row_array();
            } else {
                $proses = $this->model_app->edit('history', array('id_history' => '1', 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_history/view_history_edit', $data);
        }
    }
    ...
    function tambah_slide() {
        cek_session_akses('gallery_photos', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/front/img/slides/flexslider/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $this->load->library('upload', $config);
            ...
            if(!empty($_FILES['e']['name'])){
                ...
                $config['source_image']='asset/front/img/slides/flexslider/'.$gbr['file_name'];
                ...
                $config['width']= 1920;
                $config['height']= 768;
                $config['new_image']= 'asset/front/img/slides/flexslider/'.$gbr['file_name'];
                ...
                $data = array('username' => $this->session->username,
                    ...
                    'judul' => $this->input->post('a'),
                    'url' => seo_title($this->input->post('a')),
                    'judul_e' => $this->input->post('b'),
                    'keterangan' => $this->input->post('c'),
                    'keterangan_e' => $this->input->post('d'),
                    'gambar' => $gambar
                );
            ...
            redirect($this->uri->segment(1) . '/slide');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_slide/view_slide_tambah');
        }
    }
    ...
    function edit_slide() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_slide/view_slide_edit', $data);
        }
    ...
    function client() {
        cek_session_akses('our_client', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('our_client', 'id_client', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('our_client', array('username' => $this->session->username), 'id_client', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_client/view_client', $data);
    }

    function tambah_client() {
        cek_session_akses('our_client', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/front/img/clients/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('our_client', $data);
            redirect($this->uri->segment(1) . '/client');
        } else {
            $data['record'] = $this->model_app->view_ordering('album', 'id_album', 'DESC');
            $this->template->load('administrator/template', 'administrator/mod_client/view_client_tambah', $data);
        }
    }
    ...
    function edit_client() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_client' => $this->input->post('id'));
            $this->model_app->update('our_client', $data, $where);
            redirect($this->uri->segment(1) . '/client');
        } else {
            $record = $this->model_app->view_ordering('our_client', 'id_client', 'DESC');
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('our_client', array('id_client' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('our_client', array('id_client' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses, 'record' => $record);
            $this->template->load('administrator/template', 'administrator/mod_client/view_client_edit', $data);
        }
    }
    ...
    function delete_client() {
        cek_session_akses('our_client', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_client' => $this->uri->segment(3));
        } else {
            $id = array('id_client' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('our_client', $id);
        redirect($this->uri->segment(1) . '/client');
    }
    ...
    function footer() {
        cek_session_akses('footer', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
                $data = array(
                    'tgl_update' => date("Y-m-d H:i:s"),
                    'username' => $this->session->username,
                    'about_company' => $this->input->post('a'),
                    'information' => $this->input->post('b'),
                    'news_letter' => $this->input->post('c'),
                    'about_company_e' => $this->input->post('aa'),
                    'information_e' => $this->input->post('bb'),
                    'news_letter_e' => $this->input->post('cc'),
                    'i_telephone' => $this->input->post('d'),
                    'i_email' => $this->input->post('e')
                        );
            $where = array('id_footer' => $this->input->post('id'));
            $this->model_app->update('footer', $data, $where);
            redirect($this->uri->segment(1) . '/footer');
        } else {
            $proses = $this->model_app->edit('footer', array('id_footer' => 1))->row_array();
            $data = array('rows' => $proses);
